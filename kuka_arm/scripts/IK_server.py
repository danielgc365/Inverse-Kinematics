...
#Setting up global Parameters and symbols
...
#Setting up tables for calculating R0_6 or Rrpy
...
#The transform matrices are constructed outside of the loop to make calculations of Inverse Kinematics faster
def setupvariables():
    ...
    #The transfomr matrix from base link to end-effector with the rotation correction applied
    T_total = (T0_7 * R_corr)

# This sends the trajectory to the service, by appending the coordinates to the join_trajectory list.
# The trajectories are appended with each iteration to make the arm move step by step, I found this to be hepful as the arm
# seems to lag less
def send_calculations(req):
...
def handle_calculate_IK(req):
    ...
    # If we are done, send calculations.
    if (Total_IK_Solves == 20):
        return CalculateIKResponse(send_calculations(req))
    ...
    if len(req.poses) < 1:
        ...
        for x in xrange(0, len(req.poses)):
            ###                                                                 ###
            ###         Inverse Kinematics main calculations starts here        ###
            ###                                                                 ###
